...
class DQN(object):
    ...
    def save(self, path, i):
        torch.save(self.eval_net.state_dict(), os.path.join(path, 'eval_net_%d' % i))
        torch.save(self.target_net.state_dict(), os.path.join(path, 'target_net_%d' % i))

    def load(self, path, i):
        self.eval_net.load_state_dict(torch.load(os.path.join(path, 'eval_net_%d' % i)))
        self.eval_net.eval()

        self.target_net.load_state_dict(torch.load(os.path.join(path, 'target_net_%d' % i)))
        self.target_net.eval()

def wrap_state(state):
    return state.flatten().transpose()

if __name__ == '__main__':
    # env = gym.make('CartPole-v0')
    # env = env.unwrapped # For cheating mode to access values hidden in the environment
    env = TetrisSingleEnv(gridchoice="comboking", obs_type="diff_grid", mode="human")
    ...
    if restore_index > 0:
        dqn.load(os.path.dirname(__file__), restore_index)

    # Collect experience
    for i_episode in range(restore_index, restore_index + n_episodes):
        t = 0 # timestep
        ...
        while True:
            ...
            if done:
                ...
                if i_episode % 100 == 0 and i_episode > 0:
                    dqn.save(os.path.dirname(__file__), i_episode)
